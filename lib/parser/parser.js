...
function validateAST(node) {
  ...
  switch (node.type) {
    case 'BinaryExpression':
      ...
      // Add new validation: Check for pattern and list combination
    ...
    case 'DynamicList':
      ...
      node.elements.forEach(element => {
        ...
        if (
          !['NumericLiteral', 'StaticList', 'DynamicList'].includes(
            element.type
          )
        ) {
          throw new Error(
            `${node.type} can only contain numbers and nested lists of the same type`
          );
        }
        validateAST(element);
      });

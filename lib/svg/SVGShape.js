...
export class SVGShape {
  ...
  join(otherShape) {
    ...
    // Move both groups into new parent
    // Note: appendChild moves the element from its old location
    newGroup.appendChild(this.group);
    newGroup.appendChild(otherShape.group);

    // Create new shape with combined properties
    const joinedShape = new SVGShape(newGroup);

    // Combine animations from both shapes
    joinedShape.animations = [...this.animations, ...otherShape.animations];

    // Start with empty transform stack for the parent group
    // (child groups retain their transforms)
